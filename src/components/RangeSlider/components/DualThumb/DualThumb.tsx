...
export class DualThumb extends React.Component<DualThumbProps, State> {
  ...
  private setTrackPosition = debounce(
    () => {
      if (this.track.current) {
        const unstableGlobalTheming =
          this.context && this.context.unstableGlobalTheming;
        const thumbSize = unstableGlobalTheming ? 16 : 24;
  ...
  componentDidUpdate() {
    ...
    const unstableGlobalTheming = this.context
      ? this.context.unstableGlobalTheming
      : false;

    if (unstableGlobalTheming !== globalTheming) {
      // eslint-disable-next-line react/no-did-update-set-state
      this.setState({globalTheming: unstableGlobalTheming});
      this.setTrackPosition();
    }
  }
